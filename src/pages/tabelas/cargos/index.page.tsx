...
export default function CargosList({ data }: any) {
  ...
  return (
    <Container>
      ...
      <Box>
        <DataGridDemo columns={columns} rows={data} w="34%" />
      </Box>
      <Box>
        <Button
          title="Cadastrar"
          ...
          onClick={() => {
            router.push('/tabelas/cargos/cadastro')
          }}
        />
        ...
        <Button
          ...
          onClick={() => {
            if (selectedRowIds.length === 0) {
              ...
              router.push(`/tabelas/cargos/visualizar/${selectedRowIds}`)
            }
          }}
        />
        <Button
          ...
          onClick={() => {
            if (selectedRowIds.length === 0) {
              ...
              router.push(`/tabelas/cargos/atualizar/${selectedRowIds}`)
            }
          }}
        />
        ...
        <Modal
          title="Deletar"
          bgColor="#ff0000"
          routeDelete="/cargo/delete"
          data={selectedRowIds}
          redirectRouter="/tabelas/cargos"
        />
      </Box>
    </Container>
  )
...
export const getServerSideProps: GetServerSideProps = async () => {
  try {
    const response = await prisma.cargos.findMany()
